...
;(function ($, window, document, undefined) {
    ...
    $.extend(LinkerList.prototype, {
        getZoomScale: function () {
            ...
            const zoomParent = $(self.element).closest('.konten-soal-jawab');
            let zoom = parseFloat(zoomParent.attr('data-content-zoom'));

            if (!zoom || !isFinite(zoom) || zoom <= 0) {
                zoom = parseFloat(zoomParent.css('zoom'));
            }

            if (!zoom || !isFinite(zoom) || zoom <= 0) {
                const ranger = $('.zoom-tool-bar .ranger');
                zoom = ranger.length ? parseFloat(ranger.val()) : 1;
            }

            if (!zoom || !isFinite(zoom) || zoom <= 0) zoom = 1;
            return zoom;
        },

        getFirstItemRect: function () {
            let self = this;
            const firstChild = $('#list-kiri' + self.listIdPrefix).children()[0];
            return firstChild ? firstChild.getBoundingClientRect() : null;
        },

        getCanvasScale: function () {
            let self = this;
            if (!self.canvas) return {x: 1, y: 1};

            const canvasRect = self.canvas.getBoundingClientRect();
            const scaleX = canvasRect.width ? (self.canvas.width / canvasRect.width) : 1;
            const scaleY = canvasRect.height ? (self.canvas.height / canvasRect.height) : 1;

            return {
                x: (scaleX && isFinite(scaleX)) ? scaleX : 1,
                y: (scaleY && isFinite(scaleY)) ? scaleY : 1
            };
        },

        getItemLineY: function (item) {
            let self = this;
            const el = $(item)[0];
            if (!el) return 0;
            ...
            if (self.canvas) {
                const canvasRect = self.canvas.getBoundingClientRect();
                const scale = self.getCanvasScale();
                const visualY = (rect.top + (rect.height / 2)) - canvasRect.top;
                return (visualY * scale.y);
            }

            const firstRect = self.getFirstItemRect();
            if (!firstRect) return 0;
            return (rect.top - firstRect.top) + (rect.height / 2) + 7;
        },

        getMouseCanvasPoint: function (e) {
            let self = this;
            if (!self.canvas) return {x: e.clientX - self.offsetX, y: e.clientY - self.offsetY};

            const canvasRect = self.canvas.getBoundingClientRect();
            const scale = self.getCanvasScale();
            return {
                x: (e.clientX - canvasRect.left) * scale.x,
                y: (e.clientY - canvasRect.top) * scale.y
            };
        },

        redrawLinkedAsync: function () {
            let self = this;
            clearTimeout(self.asyncResize);
            self.asyncResize = setTimeout(function () {
        ...
        initList: function () {
            ...
            if (self.viewMode !== '1') {
                $('.item-kiri').click(function (e) {
                    ...
                    const firstRect = self.getFirstItemRect()
                    self.firstY = firstRect ? firstRect.top : 0
                    if (self.modelSoal === '1' && !self.editMode) self.drawLinked();
                    ...
                    if (!self.clicked) {
                        ...
                        const div = $(this)
                        const lineY = self.getItemLineY(div)
                ...
                $itemKanan.hover(function (el) {
                    ...
                    const div2 = $(this)
                    const lineY = self.getItemLineY(div2)
            ...
            $(document).off('content-zoom-change.linker' + self.randomId)
                .on('content-zoom-change.linker' + self.randomId, function () {
                    self.redrawLinkedAsync();
                });
        ...
        initTable: function () {
            ...
            $.each(dataTable, function (k, v) {
                if (k === 0) {
                    trs += '<tr class="bg-gray-light">';
                    $.each(v, function (key, val) {
                        if (key === 0) {
                            trs += '<th class="text-bold text-center align-middle">PERNYATAAN</th>';
                        } else {
                    ...
                    $.each(v, function (t, i) {
                        if (t === 0) {
                            trs += '<td class="baris align-middle px-2">' + self.convertLinkImage(i) + '</td>';
                        } else {
                            const selected = i === '1';
        ...
        createPoints: function () {
            ...
            self.listKiri.forEach(function (kr, idx) {
                ...
                const lineYKiri = self.getItemLineY(div)
                ...
                if (linked && linked.length > 0) {
                    ...
                    const pointKiriY = lineYKiri;
                    const pointKiriX = 15;
                    linked.forEach(function (ln) {
                        if (ln.length) {
                            const div2 = $('#item' + ln + self.listIdPrefix)
                            const lineYKanan = self.getItemLineY(div2)

                            const box = {
                                key: ln,
                                color: colorsLine[idx + 1],
                                rectBoxX: self.canvasWidth - 7,
                                rectBoxY: lineYKanan - 5,
                                lineMove: self.canvasWidth - 15,
                                lineTo: self.canvasWidth - 7,
                                lineY: lineYKanan,
                            }
        ...
        drawLinked: function () {
            ...
            const hKiri = tKiri[0] ? tKiri[0].offsetHeight : tKiri.innerHeight();
            const hKanan = tKanan[0] ? tKanan[0].offsetHeight : tKanan.innerHeight();

            const canv = $('#col-canvas' + self.listIdPrefix);
            self.canvasWidth = canv[0] ? canv[0].offsetWidth : canv.outerWidth();
            ...
            if (selCanvas.length) {
                ...
                selCanvas.attr('height', '' + Math.max(10, fixedHeight - 15));
        ...
        handleMouseMove: function (e) {
            ...
            const point = self.getMouseCanvasPoint(e);
            self.mouseX = parseInt(point.x);
            self.mouseY = parseInt(point.y);
