...
<div class="space-y-5">
    ...
    <!-- Form Container Card -->
    <form id="create" method="POST" action="{{ route('guru.bank_soal.save_bank') }}">
        @csrf
...
<script>
    ...
    $(document).ready(function () {
        ...
        function getGuruMapel(mapel) {
            if (isAdmin) {
                $.ajax({
                    url: "{{ route('guru.bank_soal.get_guru_mapel') }}?id_mapel=" + mapel,
                    type: "GET",
        ...
        function getKelasLevel(level, mapel) {
            $.ajax({
                url: "{{ route('guru.bank_soal.get_kelas_level') }}?level=" + level + "&id_guru=" + idGuru + "&mapel=" + mapel,
                type: "GET",
                success: function (data) {
                    selKelas.empty();
        ...
        $('#create').on('submit', function (e) {
            ...
            $.ajax({
                url: "{{ route('guru.bank_soal.save_bank') }}",
                type: "POST",
                dataType: "JSON",
                data: $(this).serialize(),
                headers: {
                ...
                success: function (data) {
                    if (data.status) {
                        Swal.fire({
                            ...
                            if (result.value) {
                                window.location.href = "{{ route('guru.bank_soal.index', ['type' => '0', 'mode' => '1']) }}";
                            }
                        });
                    } else {
                        Swal.fire({
