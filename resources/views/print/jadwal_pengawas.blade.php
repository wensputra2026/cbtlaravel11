...
<head>
    <meta charset="UTF-8">
    <title>Jadwal Penugasan Pengawas Ruang | {{ $appSetting->nama_aplikasi_tampil }}</title>
    <style>
...
<body>
    ...
    <div class="no-print">
        <div>
            <strong>Jadwal Penugasan Pengawas Ruang Ujian CBT</strong>
    ...
    <div class="page-a4">
        ...
        <div class="kop-surat">
            ...
            @endif
            <div class="kop-text">
                <h2>JADWAL PENUGASAN PENGAWAS RUANG UJIAN CBT</h2>
        ...
        <table class="data-table">
            <thead>
                <tr>
                    ...
                    <th>Hari / Tanggal</th>
                    <th style="width: 80px;">Sesi / Waktu</th>
                    <th>Mata Pelajaran</th>
                    <th style="width: 75px;">Ruang</th>
                    <th>Nama Pengawas Ruang</th>
                    <th>NIP</th>
                </tr>
            </thead>
            <tbody>
                @forelse($pengawas as $idx => $p)
                    <tr>
                        ...
                        <td>
                            <strong>{{ $p->jadwal?->tgl_mulai ? date('d/m/Y', strtotime($p->jadwal->tgl_mulai)) : 'Sesuai Jadwal' }}</strong>
                        </td>
                        <td style="text-align: center;">
                            <div>{{ $p->sesi?->nama_sesi ?? 'Sesi 1' }}</div>
                            <div style="font-size: 9px; color: #666;">{{ $p->sesi?->waktu_mulai ? substr($p->sesi->waktu_mulai, 0, 5) : '' }} - {{ $p->sesi?->waktu_akhir ? substr($p->sesi->waktu_akhir, 0, 5) : '' }}</div>
                        </td>
                        <td>{{ $p->jadwal?->bankSoal?->mapel?->nama_mapel ?? 'Ujian CBT' }}</td>
                        <td style="text-align: center;"><strong>{{ $p->ruang?->nama_ruang ?? 'Ruang 01' }}</strong></td>
                        <td><strong>{{ $p->guru?->nama_guru ?? 'Pengawas Ruang' }}</strong></td>
                        <td style="font-size: 10px; font-family: monospace;">{{ $p->guru?->nip ?? '-' }}</td>
                    </tr>
                @empty
                    <tr>
