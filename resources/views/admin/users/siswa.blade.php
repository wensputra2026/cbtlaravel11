...
<div class="space-y-6" x-data="{ openResetModal: false, selectedUserId: null, selectedUsername: '' }">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
            <h3 class="text-base font-bold text-slate-800">Daftar Akun Peserta Ujian (Siswa)</h3>
            <p class="text-xs text-slate-500">Kelola akun kredensial login peserta CBT, cetak kartu massal, dan reset kata sandi</p>
        </div>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
        <form action="{{ route('admin.users.siswa') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
                <label class="block text-[11px] font-semibold text-slate-500 mb-1">Filter Rombel / Kelas</label>
            ...
            <div class="sm:col-span-2">
                <label class="block text-[11px] font-semibold text-slate-500 mb-1">Pencarian Nama / Username / NISN</label>
                <div class="flex gap-2">
                    <input type="text" name="q" value="{{ $search }}" placeholder="Ketik nama, username atau NISN siswa..." class="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500">
                    ...
                    @if($search || $kelasId)
                        <a href="{{ route('admin.users.siswa') }}" class="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-xs font-bold transition">Reset</a>
                    @endif
                </div>
            </div>
        </form>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-xs text-slate-600">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($siswas as $idx => $s)
                        ...
                        <tr class="hover:bg-slate-50 transition">
                            ...
                            <td class="py-3.5 px-4">
                                ...
                                <div class="text-[11px] text-slate-500 font-mono">NISN: {{ $s->nisn ?? '-' }}</div>
                            </td>
                            <td class="py-3.5 px-4">
                            ...
                            <td class="py-3.5 px-4 font-mono font-bold text-brand-600">{{ $s->username }}</td>
                            <td class="py-3.5 px-4 font-mono text-slate-600 font-semibold">{{ $s->password ?? '123456' }}</td>
                            <td class="py-3.5 px-4 text-center">
                                @if($u && $u->active)
                                    <span class="px-2 py-0.5 text-[10px] font-bold rounded-full bg-emerald-50 text-emerald-600 border border-emerald-200">
                                        Aktif
                                    </span>
                                @elseif($u)
                                    <span class="px-2 py-0.5 text-[10px] font-bold rounded-full bg-rose-50 text-rose-600 border border-rose-200">
                                        Nonaktif
                                    </span>
                                @else
                                    <span class="px-2 py-0.5 text-[10px] font-bold rounded-full bg-slate-100 text-slate-500">
                                        Tersimpan
                                    </span>
                                @endif
                            </td>
                            <td class="py-3.5 px-4 text-center">
                                @if($u)
                                    <button type="button" @click="selectedUserId = {{ $u->id }}; selectedUsername = '{{ $s->username }}'; openResetModal = true" class="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-[11px] font-semibold transition" title="Reset Password">
                                        Reset
                                    </button>
                                @else
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="py-8 text-center text-slate-500">Tidak ada akun peserta ujian ditemukan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div x-show="openResetModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" style="display: none;" x-cloak>
        <div class="bg-white border border-slate-200 rounded-2xl p-6 w-full max-w-sm shadow-xl" @click.away="openResetModal = false">
            <h4 class="text-sm font-bold text-slate-800 mb-1">Reset Password Akun Siswa</h4>
