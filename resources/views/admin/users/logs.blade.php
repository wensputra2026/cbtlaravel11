...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h3 class="text-base font-bold text-slate-800">Rekam Jejak Aktivitas (Audit Log)</h3>
            <p class="text-xs text-slate-500">Pencatatan login, perubahan konfigurasi, serta aktivitas operasional CBT secara real-time</p>
        </div>
        <div>
            <form action="{{ route('admin.setting.database.clear') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin mengosongkan seluruh riwayat log aktivitas?')">
                ...
                <button type="submit" class="px-3.5 py-2 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 rounded-xl text-xs font-bold transition flex items-center gap-1.5 shadow-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($logs as $idx => $log)
                        <tr class="hover:bg-slate-50 transition">
                            <td class="py-3.5 px-4 text-slate-500">{{ is_object($logs) && method_exists($logs, 'firstItem') ? $logs->firstItem() + $idx : $idx + 1 }}</td>
                            ...
                            <td class="py-3.5 px-4 font-bold text-slate-800">
                                ...
                                @if($log->username)
                                    <div class="text-[10px] font-mono text-brand-600">{{ $log->username }}</div>
                                @endif
                            </td>
                            ...
                            <td class="py-3.5 px-4">
                                <div class="text-slate-800 font-medium">{{ $log->log_desc ?? '-' }}</div>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-8 text-center text-slate-500">Belum ada riwayat aktivitas yang tercatat.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
