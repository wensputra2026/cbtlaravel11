...
<div class="space-y-6" x-data="{ openResetModal: false, selectedUserId: null, selectedUsername: '' }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h3 class="text-base font-bold text-slate-800">Daftar Akun Guru & Pengawas</h3>
            <p class="text-xs text-slate-500">Pengelolaan hak akses portal guru, pembuatan bank soal, dan pengawas ujian</p>
        </div>
        <div class="w-full sm:w-80">
            <form action="{{ route('admin.users.guru') }}" method="GET">
                <input type="text" name="q" value="{{ $search }}" placeholder="Cari nama, NIP, atau username guru..." class="w-full px-3.5 py-2 bg-white border border-slate-200 rounded-xl text-xs text-slate-800 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 shadow-sm">
            </form>
        </div>
    </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($gurus as $idx => $g)
                        ...
                        <tr class="hover:bg-slate-50 transition">
                            <td class="py-3.5 px-4 text-slate-500">{{ $gurus->firstItem() + $idx }}</td>
                            <td class="py-3.5 px-4">
                                <div class="font-bold text-slate-800">{{ $g->nama_guru }}</div>
                                <div class="text-[11px] text-slate-500 font-mono">NIP: {{ $g->nip ?: '-' }}</div>
                            </td>
                            <td class="py-3.5 px-4 font-mono font-bold text-brand-600">{{ $g->username }}</td>
                            ...
                            <td class="py-3.5 px-4 text-center">
                                ...
                                @else
                                    <span class="px-2.5 py-1 text-[10px] font-bold rounded-full bg-slate-100 text-slate-500">
                                        Belum Sinkron
                                    </span>
                                @endif
                            </td>
                            <td class="py-3.5 px-4 text-center">
                                <div class="flex items-center justify-center gap-1.5">
                                    @if($u)
                                        <button type="button" @click="selectedUserId = {{ $u->id }}; selectedUsername = '{{ $g->username }}'; openResetModal = true" class="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-[11px] font-semibold transition" title="Reset Password">
                                            Reset Password
                                        </button>
                                        <form action="{{ route('admin.users.toggle_active', $u->id) }}" method="POST" class="inline" onsubmit="return confirm('Ubah status aktif akun ini?')">
                                            @csrf
                                            <button type="submit" class="px-2.5 py-1 {{ $u->active ? 'bg-rose-50 text-rose-600 hover:bg-rose-100' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100' }} rounded-lg text-[11px] font-semibold transition">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-8 text-center text-slate-500">Tidak ada data akun guru ditemukan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div x-show="openResetModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" style="display: none;" x-cloak>
        <div class="bg-white border border-slate-200 rounded-2xl p-6 w-full max-w-sm shadow-xl" @click.away="openResetModal = false">
            <h4 class="text-sm font-bold text-slate-800 mb-1">Reset Password Akun Guru</h4>
