...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    <span class="text-xs font-semibold text-slate-500">Nama Database</span>
                    <h3 class="text-lg font-mono font-black text-slate-800 mt-1">{{ $dbName }}</h3>
                </div>
                <div class="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4"/></svg>
                </div>
            </div>
            <p class="text-[11px] text-slate-500 mt-2">Koneksi aktif MySQL / MariaDB</p>
        </div>

        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    <span class="text-xs font-semibold text-slate-500">Total Tabel</span>
                    <h3 class="text-lg font-black text-slate-800 mt-1">{{ $tableCount }} <span class="text-xs font-normal text-slate-500">tabel</span></h3>
                </div>
            ...
            <p class="text-[11px] text-slate-500 mt-2">Struktur skema CBT murni</p>
        </div>

        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    <span class="text-xs font-semibold text-slate-500">Ukuran Penyimpanan</span>
                    <h3 class="text-lg font-black text-slate-800 mt-1">{{ $sizeMb }} <span class="text-xs font-normal text-slate-500">MB</span></h3>
                </div>
                <div class="w-10 h-10 rounded-xl bg-purple-50 text-purple-600 flex items-center justify-center">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                </div>
            </div>
            <p class="text-[11px] text-slate-500 mt-2">Data & indeks gabungan</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
            <div>
                <div class="flex items-center gap-3 mb-3">
                    ...
                    <div>
                        <h4 class="text-sm font-bold text-slate-800">Backup Database Lengkap (.SQL)</h4>
                        <p class="text-xs text-slate-500">Ekspor seluruh tabel, data master, bank soal, dan hasil ujian</p>
                    </div>
                </div>
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
            <div class="flex items-center gap-3 mb-3">
                ...
                <div>
                    <h4 class="text-sm font-bold text-slate-800">Pembersihan Sesi & Pemeliharaan</h4>
                    <p class="text-xs text-slate-500">Optimalisasi performa server dan pembersihan data berkala</p>
                </div>
            </div>
            ...
            <div class="space-y-3 pt-1">
                <form action="{{ route('admin.setting.database.clear') }}" method="POST" onsubmit="return confirm('Reset seluruh kunci perangkat (device lock) siswa?')">
                    ...
                    <button type="submit" class="w-full flex items-center justify-between px-3.5 py-2.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl text-xs font-semibold text-slate-700 transition">
                        <span>🔓 Reset Kunci Perangkat Siswa (Device Lock)</span>
                        <span class="text-[10px] text-brand-600 font-bold">Eksekusi &rarr;</span>
                    </button>
                </form>
                ...
                <form action="{{ route('admin.setting.database.clear') }}" method="POST" onsubmit="return confirm('Kosongkan riwayat log aktivitas pengguna?')">
                    ...
                    <button type="submit" class="w-full flex items-center justify-between px-3.5 py-2.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl text-xs font-semibold text-slate-700 transition">
                        ...
                        <span class="text-[10px] text-amber-600 font-bold">Eksekusi &rarr;</span>
                    </button>
                </form>
                ...
                <form action="{{ route('admin.setting.database.clear') }}" method="POST" onsubmit="return confirm('Hapus seluruh sesi ujian yang berstatus SELESAI? Data nilai yang sudah direkap tetap aman.')">
                    ...
                    <button type="submit" class="w-full flex items-center justify-between px-3.5 py-2.5 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-xl text-xs font-semibold text-rose-700 transition">
                        <span>🗑️ Bersihkan Sesi Ujian Berstatus Selesai</span>
                        <span class="text-[10px] text-rose-600 font-bold">Eksekusi &rarr;</span>
                    </button>
                </form>
            </div>
        </div>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
        <h4 class="text-sm font-bold text-slate-800 mb-1">Daftar Status Tabel Database</h4>
        <p class="text-xs text-slate-500 mb-4">Pemeriksaan ukuran record data dan engine database MySQL saat ini</p>
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach($tables as $idx => $t)
                        <tr class="hover:bg-slate-50 transition">
                            <td class="py-2.5 px-4 text-slate-500">{{ $idx + 1 }}</td>
                            <td class="py-2.5 px-4 font-mono font-bold text-slate-800">{{ $t->Name }}</td>
                            ...
                            <td class="py-2.5 px-4 text-slate-500 text-[11px]">{{ $t->Collation }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
